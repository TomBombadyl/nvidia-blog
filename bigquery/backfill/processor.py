"""
Processor for Backfill
Orchestrates reading cleaned text, chunking, embedding, and writing to BigQuery.
"""
...
class BackfillProcessor:
    """Processes cleaned text files and writes to BigQuery."""
    ...
    def __init__(self):
        ...
        logger.info("Initialized backfill processor")
    ...
    def skip_already_processed(self, item_ids: List[str], feed: str) -> List[str]:
        """
        Filter out item IDs that are already in BigQuery with publication_date.
        Re-process items that have NULL publication_date.
        ...
        """
        try:
            # Get items that need re-processing (NULL publication_date or not in DB)
            query = f"""
            SELECT DISTINCT item_id
            FROM `{self.bq_client.items_table_ref}`
            WHERE feed = @feed AND publication_date IS NOT NULL
            """
            ...
            processed_with_date = {row.item_id for row in results}
            # Re-process items that are missing or have NULL publication_date
            new_ids = [item_id for item_id in item_ids if item_id not in processed_with_date]
            
            logger.info(
                f"Feed {feed}: {len(new_ids)} items to process (missing or NULL publication_date) out of {len(item_ids)} total"
            )
            
            return new_ids
            
        except Exception as e:
            logger.error(f"Error checking processed items: {e}")
            # If we can't check, process all items (idempotent anyway)
            return item_ids
